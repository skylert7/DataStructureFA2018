...
public:    
    ...
    DSvector(unsigned int size);            //CONSTRUCTOR WITH SIZE
    DSvector(const DSvector<T> &object);    //COPY CONSTRUCTOR
    ~DSvector();                            //DESTRUCTOR

    unsigned int capacity() const;                   //Get capacity of vector
    unsigned int size() const;                       //Get size of vector

    T at(unsigned int index);                        //Get element at specific index
    ...
    void erase(unsigned int index);                  //Erase element in specific index
    void swap(unsigned int index1, unsigned int index2);      //Swap 2 elements between two different indexes
	void sort();							//Sort element alphabetically (Ascending order)
	void print();                           //Print element of the vector (ONLY WORKS ON PRIMITIVE TYPE)

    T &operator[](unsigned int index);               //Get reference of element of specific index
...
template<class T>
//CONSTRUCTOR
DSvector<T>::DSvector(unsigned int size)
{
    sizeOfVector = 0;
    if(size > 0)
        my_capacity = size * 2;
...
template<class T>
unsigned int DSvector<T>::capacity() const
{
    return my_capacity;
}

template<class T>
T DSvector<T>:: at(unsigned int index)
{
...
template<class T>
void DSvector<T>::erase(unsigned int index)
{
    //Check if index is valid
	if (index > sizeOfVector - 1)
    ...
    {
        for(unsigned int i = index; i < sizeOfVector; i++)
        {
            if(i != (sizeOfVector - 1))
...
template<class T>
void DSvector<T>::swap(unsigned int index1,unsigned int index2)
{
    T dummy;
...
template<class T>
unsigned int DSvector<T>::size() const
{
...
template<class T>
T &DSvector<T>::operator[](unsigned int index)
{
    if(index > sizeOfVector - 1)
        throw out_of_range("Out Of Range.\n");
    else
